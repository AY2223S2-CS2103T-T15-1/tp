...
import seedu.address.obsoletemodel.Model;

/**
 * Changes the remark of an existing entity in the address book.
 */
...
``` java
    public static final String MESSAGE_USAGE = COMMAND_WORD
            + ": Edits the remark of the entity identified "
            + "by the index number used in the last entity listing. "
...
public class RemarkCommand extends Command {
    ...
    /**
     * @param index of the entity in the filtered entity list to edit the remark
     * @param remark of the entity to be updated to
     */
...
Create a new `Remark` in `seedu.address.obsoletemodel.entity`. Since a `Remark` is a field that is similar to `Address`, we can reuse a significant bit of code.
...
Without getting too deep into `fxml`, let’s go on a 5 minute adventure to get some placeholder text to show up for each entity.
...
``` java
public PersonCard(Person entity, int displayedIndex) {
    //...
    remark.setText(entity.getRemark().value);
}
```
...
//...
    ...
    public CommandResult execute(Model model) throws CommandException {
        ...
        Person entityToEdit = lastShownList.get(index.getZeroBased());
        Person editedEntity = new Person(
                entityToEdit.getName(), entityToEdit.getPhone(), entityToEdit.getEmail(),
                entityToEdit.getAddress(), remark, entityToEdit.getTags());

        model.setPerson(entityToEdit, editedEntity);
        model.updateFilteredPersonList(PREDICATE_SHOW_ALL_PERSONS);

        return new CommandResult(generateSuccessMessage(editedEntity));
    }
     ...
     * {@code entityToEdit}.
     */
    private String generateSuccessMessage(Person entityToEdit) {
        String message = !remark.value.isEmpty() ? MESSAGE_ADD_REMARK_SUCCESS : MESSAGE_DELETE_REMARK_SUCCESS;
        return String.format(message, entityToEdit);
    }
```
